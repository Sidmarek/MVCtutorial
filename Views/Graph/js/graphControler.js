    ...
    function getConfig() {    
    var url_1 = '/graphs_get_config2?' + d.getTime();
      $.ajax({ url: url_1,
    		async: true,
        //type: "post",
    		dataType: "json",
    ...
    // načtení konfigurace ze serveru             
    function getConfig() {
    var url_1 = 'http://192.168.2.26/api/Graph/getConfig';    
      $.ajax({ url: url_1,
    		async: true,        
    		dataType: "json",
        type: "post",
    ...
    dataRequest = {"beginTime": 537962399, "timeAxisLength": 86400, "chartWidth": 1440, "tags": [{"period": 30, "table": "norm", "column": "iWMU_Temp"},
                                                                                                 {"period": 30, "table": "norm", "column": "iMF1_Temp"},
                                                                                                 {"period": 30, "table": "norm", "column": "iMF2_Temp"},
                                                                                                 {"period": 30, "table": "norm", "column": "diMF1_Weight"},
                                                                                                 {"period": 30, "table": "norm", "column": "diMF2_Weight"},
                                                                                                 {"period": 60, "table": "slow", "column": "iMF1_Status"},
                                                                                                 {"period": 60, "table": "slow", "column": "iMF2_Status"},
                                                                                                 {"period": 60, "table": "slow", "column": "iMF1_Step"},
                                                                                                 {"period": 60, "table": "slow", "column": "iMF2_Step"}
                                                                                                ]
                   };
                   
    dataRequest = "";
    
    dataRequest += '{"beginTime":' + beginTime + ', "timeAxisLength":' + timeAxisLength + ', "chartWidth": ' + chartWidth + ', "tags":[';
            
      for (var f=0;f<config.view[view].field.length;f++) {
        for (var s=0;s<config.view[view].field[f].signals.length;s++) {
          signal = config.view[view].field[f].signals[s];
          
          dataRequest += '{"table":' + signal.table + ', "column": ' + signal.column;
          
          optTimeStep = timeAxisLength/(2*chartWidth);
          ...
          dataRequest += dataRequest.substr(0,dataRequest-1);
          dataRequest += ']';
        };
      };
    ...
    var url = '/graph/getData&' + d.getTime();
    ...
    var viewInfo = new Array(),
      ...
      for (var field=1;field<=config.view[view]["field"].length;field++) {        
        ...
        signals = config.view[view]["field"][field-1]["signals"];
